...
import code.elix_x.mods.btam.core.horses.ai.EntityAIRunAroundLikeCrazy;

import com.google.common.base.Predicate;

public class NewEntityHorse extends EntityAnimal /*EntityHorse*/ implements IInvBasic
{
	private static final Predicate horseBreedingSelector = new Predicate()
	{
		...
		{
			return p_179873_1_ instanceof NewEntityHorse && ((NewEntityHorse)p_179873_1_).func_110205_ce();
		}
	...
	//    private static final String[] shortHorseArmorTextures = new String[] {"", "meo", "goo", "dio"};
	...
	private static final String[] shortHorseTextures = new String[] {"hwh", "hcr", "hch", "hbr", "hbl", "hgr", "hdb"};
	...
	private static final String[] shortHorseMarkingTextures = new String[] {"", "wo_", "wmo", "wdo", "bdo"};
	...
	public NewEntityHorse(World worldIn)
	{
		...
		this.func_110226_cD();
		//		        applyArmorCalculations(p_70655_1_, p_70655_2_)
	}

	public NewEntityHorse(EntityHorse oldHorse){
		super(oldHorse.worldObj);
		this.setSize(1.4F, 1.6F);
		this.isImmuneToFire = oldHorse.isImmuneToFire();
		this.setChested(oldHorse.isChested());
		((PathNavigateGround)this.getNavigator()).func_179690_a(true);
		this.tasks.addTask(0, new EntityAISwimming(this));
		this.tasks.addTask(1, new EntityAIPanic(this, 1.2D));
		this.tasks.addTask(1, new EntityAIRunAroundLikeCrazy(this, 1.2D));
		...
		rotationPitch = oldHorse.rotationPitch;
		rotationYaw = oldHorse.rotationYaw;
		rotationYawHead = oldHorse.rotationYawHead;
		
		setHorseType(oldHorse.getHorseType());
		setHorseVariant(oldHorse.getHorseVariant());
		entityAge = oldHorse.getAge();
		setHealth(oldHorse.getHealth());
//		setJumpPower();
	}
	...
	private int getHorseArmorIndex(ItemStack itemstack)
	{
		if(itemstack == null)
			return -1;
		return HorseArmorRegistry.getId(itemstack.getItem());
	}

	public int getTotalArmorValue()
	{
		if(this.getArmorId() == -1)
			return 0;
		return HorseArmorRegistry.getMaterial(this.getArmorId()).getDamageReductionAmount();
	}
	...
	protected void damageArmor(float amount) {
		...
		{
			horseChest.getStackInSlot(1).damageItem((int)amount, this);
		...
		else
		{
			if (itemstack != null)
			{
				...
				if (this.canWearArmor())
				{
					...
					if(HorseArmorRegistry.isArmorValid(itemstack)){
						if (!this.isTame())
						{
	...
	{
		return HorseArmorRegistry.isArmorValid(item);
	}
	...
	public boolean replaceItemInInventory(int p_174820_1_, ItemStack p_174820_2_)
	{
		...
		if (j >= 0 && j < 2 && j < this.horseChest.getSizeInventory())
		{
			...
			else if (j == 1 && (p_174820_2_ != null && !HorseArmorRegistry.isArmorValid(p_174820_2_.getItem()) || !this.canWearArmor()))
			{
				return false;
			}
			else
	...
	{
//		return this.dataWatcher.getWatchableObjectInt(22);
		System.out.println("Does slot 1 has nothing: " + (horseChest.getStackInSlot(1) == null) + " (" + (worldObj.isRemote ? "client" : "server") + ")");
		if(horseChest.getStackInSlot(1) != null){
			System.out.println("Slot 1 has: " + horseChest.getStackInSlot(1).getDisplayName() + " (" + (worldObj.isRemote ? "client" : "server") + ")");
		}
		System.out.println("Slot 1 has armor with id: " + HorseArmorRegistry.getId(horseChest.getStackInSlot(1)) + " (" + (worldObj.isRemote ? "client" : "server") + ")");
		return HorseArmorRegistry.getId(horseChest.getStackInSlot(1));
	}*/

	public void updateArmorAndSaddle()
	{
	...
	private void setHorseTexturePaths()
	{
		...
		this.layeredTexture[0] = null;
		this.layeredTexture[1] = null;
		this.layeredTexture[2] = null;
		...
		if (i == 0)
		{
			...
			this.layeredTexture[0] = horseTextures[k];
			this.texturePrefix = this.texturePrefix + shortHorseTextures[k];
			...
			this.layeredTexture[1] = horseMarkingTextures[l];
			this.texturePrefix = this.texturePrefix + shortHorseMarkingTextures[l];
		}
		else
		{
			this.layeredTexture[0] = "";
		...
		} else if(HorseArmorRegistry.getItem(k) != null){
			this.layeredTexture[2] = HorseArmorRegistry.getMaterial(k).getTextureName();
			this.texturePrefix = this.texturePrefix + HorseArmorRegistry.getMaterial(k).getShortTextureName();
			this.field_175508_bO = true;
		} else {
			this.field_175508_bO = false;
			return;
	...
	public String[] getVariantTexturePaths()
	{
		...
		return this.layeredTexture;
	}

	/**=============================== ANOTHER CHANGED STUFF ===============================**/
	...
	public void openGUI(EntityPlayer player)
	{
		if (!this.worldObj.isRemote && (this.riddenByEntity == null || this.riddenByEntity == player) && this.isTame())
		{
			...
			FMLNetworkHandler.openGui(player, BTAMBase.INSTANCE, BTAMBase.newHorseGui, worldObj, this.getEntityId(), 0, 0);
		}
	}

	/**=============================== OTHER STUFF DO NOT TUCH ===============================**/
	...
	private void func_110226_cD()
	{
		...
		this.horseChest = new AnimalChest("HorseChest", this.getInventorySize());
		this.horseChest.setCustomName(this.getName());
	...
	{
		int i = this.getArmorId();
		boolean flag = this.isHorseSaddled();
		this.updateArmorAndSaddle();

		if (this.ticksExisted > 20)
		{
			if (i == 0 && i != this.getArmorId())
			{
				this.playSound("mob.horse.armor", 0.5F, 1.0F);
			}
			else if (i != this.getArmorId())
			{
				this.playSound("mob.horse.armor", 0.5F, 1.0F);
			}
	...
	protected NewEntityHorse getClosestHorse(Entity p_110250_1_, double p_110250_2_)
	{
	...
	@SideOnly(Side.CLIENT)
	public boolean renderPartially()
	{
		return this.getHorseType() == 0 || this.getArmorId() >= 0;
	}

	@SideOnly(Side.CLIENT)
	public boolean func_175508_bO()
	{
		return this.field_175508_bO;
	}

	private void mountPlayer(EntityPlayer p_110237_1_)
	{
	...
	public void onLivingUpdate()
	{
		...
		if (!this.worldObj.isRemote)
		{
			...
			{
				NewEntityHorse NewEntityHorse = this.getClosestHorse(this, 16.0D);

				if (NewEntityHorse != null && this.getDistanceSqToEntity(NewEntityHorse) > 4.0D)
				{
					this.navigator.getPathToEntityLiving(NewEntityHorse);
				}
			}
		}
	}
	...
	public boolean canMateWith(EntityAnimal otherAnimal)
	{
		...
		{
			NewEntityHorse NewEntityHorse = (NewEntityHorse)otherAnimal;

			if (this.canMate() && NewEntityHorse.canMate())
			{
				int i = this.getHorseType();
				int j = NewEntityHorse.getHorseType();
	...
	{
		NewEntityHorse NewEntityHorse = (NewEntityHorse)ageable;
		NewEntityHorse NewEntityHorse1 = new NewEntityHorse(this.worldObj);
		int i = this.getHorseType();
		int j = NewEntityHorse.getHorseType();
		int k = 0;
		...
		if (k == 0)
		{
			...
			{
				l = NewEntityHorse.getHorseVariant() & 255;
			}
			else
			{
			...
			{
				l |= NewEntityHorse.getHorseVariant() & 65280;
			}
			else
			{
			...
			NewEntityHorse1.setHorseVariant(l);
		}

		NewEntityHorse1.setHorseType(k);
		...
		NewEntityHorse1.getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(d1 / 3.0D);
		...
		NewEntityHorse1.getEntityAttribute(horseJumpStrength).setBaseValue(d2 / 3.0D);
		...
		NewEntityHorse1.getEntityAttribute(SharedMonsterAttributes.movementSpeed).setBaseValue(d0 / 3.0D);
		return NewEntityHorse1;
	}
	...
	public IEntityLivingData func_180482_a(DifficultyInstance p_180482_1_, IEntityLivingData p_180482_2_)
	{
		...
		if (p_180482_2_1 instanceof NewEntityHorse.GroupData)
		{
			l = ((NewEntityHorse.GroupData)p_180482_2_1).field_111107_a;
			i = ((NewEntityHorse.GroupData)p_180482_2_1).field_111106_b & 255 | this.rand.nextInt(5) << 8;
		}
		else
		{
			...
			p_180482_2_1 = new NewEntityHorse.GroupData(l, i);
		}
